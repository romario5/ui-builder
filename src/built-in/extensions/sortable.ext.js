    ...
    Extension.register('Sortable', {
        ...
        onApply(ext) {
            ...
            params.dragStartHandler = function (inst) {
                ...
                initialOpacity = el.node().style.opacity;
                el.css({opacity: 0.25});
            };
            ...
            params.dragHandler = function(inst, initialPoint, deltaPoint, event) {
                ...
                for (let i = 0, len = items.length; i < len; i++) {
                    ...
                    // Before.
                    if ((i === 0 && newY < rect.top + halfH) || (newY < rect.top + halfH && newY > rect.top - halfH)) {
                        if (item.node().previousSibling === this.node()) break;
                    ...
                    // After.
                    } else if ((i === len - 1 && newY > rect.top + halfH) || (newY > rect.top + halfH && newY < rect.top + rect.height + halfH)) {
                        if (item.node().nextSibling === this.node()) break;
                        placeholder = document.createElement('div');
                        placeholder.style.height = item.outerHeight() + 'px';
                        placeholder.style.width = '1px';
            ...
            params.dropHandler = function(inst) {
                ...
                if (placeholder !== null) {
                    ...
                    target.node().replaceChild(this.node(), placeholder);
                    // target.node().removeChild(placeholder);
                    placeholder = null;
                    target.triggerEvent('sort', this);
                }
